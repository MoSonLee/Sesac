...
//        sendNotification()
    ...
    //Notification 3. 권한을 허용한 사용자에게 알림 요청(언제? 어떤 콘텐츠?)
    ...
    func sendNotification() {
       let notificationContent = UNMutableNotificationContent()
        ...
        //매 시간 15분에 알림을 주고 싶을 때
        var datacomponent = DateComponents()
        datacomponent.minute = 15
        //언제 보낼것인가? 1. 시간 간격 2. 캘린더 3. 위치에 따라 설정 가능.
        let trigger = UNTimeIntervalNotificationTrigger(timeInterval: 60, repeats: true)
        let trigger2 = UNCalendarNotificationTrigger(dateMatching: datacomponent, repeats: true)
        // 알림 요청
